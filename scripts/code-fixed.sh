...
function code() {
	...
	if [[ "$OSTYPE" == "darwin"* ]]; then
		# Use the correct path for VS Aware.app
	...
	# Disable problematic extensions to avoid native module issues
	DISABLE_EXTENSIONS="--disable-extension=vscode.vscode-api-tests"
	if [[ "$@" == *"--extensionTestsPath"* ]]; then
		DISABLE_EXTENSIONS=""
	fi

	# Launch Code with disabled extensions to avoid policy service issues
